...
namespace Harjoitustyön_kokeillu
    {
    /// <summary>
    /// Interaction logic for laskuNäkymä.xaml
    /// </summary>
    public partial class laskuNäkymä : Window
    {
        public int counter = 0;
        
        public int asiakkaanID = 0;
        public int  Total = 0;
        ...
        public ObservableCollection<LaskuTiedot> ShowBill (int asiakasid)
            {
            ...
            using (MySqlConnection connection = new MySqlConnection("Server=127.0.0.1; Port=3306; User ID=opiskelija; Pwd=opiskelija1; Database=laskutusdb;"))
                ...
                string createTableQuery = @"     
                                    ...
                                    WHERE l.LaskunNumero = lv.LaskunNumero AND l.AsiakasId = s.Id AND s.Id = @id";
                MySqlCommand createDatabaseCommand = new MySqlCommand(createTableQuery, connection);
                createDatabaseCommand.Parameters.AddWithValue("@id", asiakasid);
        ...
        private void laskee (object sender, RoutedEventArgs e)
            ...
            if (counter != 0)
                {
                counter--;
                ShowBill(asiakkaanID);
                }
            }

        private void kasvaa (object sender, RoutedEventArgs e)
            {
            if (counter < ShowBill(asiakkaanID).Count() - 1)
                {
                counter++;
                ShowBill(asiakkaanID);
                }
